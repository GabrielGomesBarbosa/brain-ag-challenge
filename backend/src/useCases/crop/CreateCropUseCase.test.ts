...
describe('CreateCropUseCase', () => {
  ...
  it('should throw validation error when input is invalid', async () => {
    ...
    await expect(createCropUseCase.execute(invalidInput)).rejects.toThrow(ZodError);
  });
  ...
  it('should throw an error when slug already exists', async () => {
    ...
    await expect(createCropUseCase.execute(input)).rejects.toThrow('Unique constraint failed');
